...
\centering
\small
\caption{Cross-platform category-level Wang parameter comparison. Each cell reports $\hat{\lambda}_{MLE}$ estimated separately by category. $\Delta\lambda = \hat{\lambda}_{\text{Kalshi}} - \hat{\lambda}_{\text{Polymarket}}$; positive values indicate higher risk loading on the less liquid platform. Significance: $^{***}p<0.001$, $^{**}p<0.01$, $^{*}p<0.05$.}
